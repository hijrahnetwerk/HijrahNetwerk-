(function(){
  ...
  function addField(form,id,label,wide){
    ...
    wrap.innerHTML='<label style="display:block;font-size:12px;margin-bottom:5px;color:#786f66">'+esc(label)+'</label>'+
      (wide?'<textarea id="'+id+'" rows="3" style="width:100%;padding:10px;border:1px solid #e8e1d8;border-radius:9px"></textarea>':
      '<input id="'+id+'" style="width:100%;padding:10px;border:1px solid #e8e1d8;border-radius:9px">');
  ...
  async function mountFiche(topic){
    ...
    card.innerHTML='<summary style="cursor:pointer;font-weight:700;color:#4d3819">Aanvullen of corrigeren</summary><div style="padding-top:12px"><p class="card-sub">Vul alleen in wat jij zelf wilt toevoegen of corrigeren. De bestaande HN-gegevens worden hier bewust niet getoond. Dit is een bewerkvoorstel voor HN en past de originele fiche niet rechtstreeks aan.</p><form id="hnEditFicheForm" style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px"></form></div>';
    app.appendChild(card);
    const form=card.querySelector('#hnEditFicheForm');
    const fields=[
      ['name','Naam / titel'],['full_name','Volledige naam'],
      ['practice_name','Praktijk / organisatie'],['service_type','Type dienst'],
      ['specialization','Specialisatie'],['for_children','Voor kinderen'],
      ['languages','Talen'],['phone','Telefoon'],['email','E-mail'],
      ['website','Website'],['address','Adres'],['neighborhood','Wijk'],
      ['opening_hours','Openingstijden',true],['appointment','Afspraak nodig'],
      ['home_visits','Huisbezoeken'],['teleconsultation','Teleconsultatie'],
      ['price','Prijs / kosten'],['insurance','Verzekering / CNSS'],
      ['emergency','Spoed'],['short_description','Korte beschrijving',true]
    ];
    fields.forEach(f=>addField(form,'hn_'+f[0],f[1],!!f[2]));
    const extra=document.createElement('div');extra.style.gridColumn='1/-1';
    extra.innerHTML='<label style="display:block;font-size:12px;margin-bottom:5px;color:#786f66">Wat wil je nog meegeven?</label><textarea id="hn_edit_message" rows="3" style="width:100%;padding:10px;border:1px solid #e8e1d8;border-radius:9px" placeholder="Wat wil je toevoegen? Je kunt hier ook je bron of eigen ervaring toelichten."></textarea>';
    ...
    form.addEventListener('submit',async e=>{
      ...
      fields.forEach(f=>{
        ...
        if(v)proposed[f[0]==='name'?'full_name':f[0]]=v;
      });
  ...
  async function mountArticle(topic){
    ...
    details.innerHTML='<summary style="cursor:pointer;font-weight:700;color:#fff">Artikel aanvullen of corrigeren</summary><div style="padding-top:14px"><p>Vul alleen nieuwe informatie in die je zelf wilt toevoegen of corrigeren. De bestaande HN-tekst wordt hier bewust niet getoond.</p><form id="hnEditArticleForm" style="display:grid;gap:10px"><input id="hn_article_title" placeholder="Eventueel een nieuwe titel" style="padding:10px;border-radius:9px;border:1px solid #e8e1d8"><input id="hn_article_summary" placeholder="Eventueel een nieuwe of aanvullende samenvatting" style="padding:10px;border-radius:9px;border:1px solid #e8e1d8"><textarea id="hn_article_content" rows="8" placeholder="Welke informatie wil je toevoegen of corrigeren?" style="padding:10px;border-radius:9px;border:1px solid #e8e1d8"></textarea><input id="hn_article_source" placeholder="Bron, als je die hebt" style="padding:10px;border-radius:9px;border:1px solid #e8e1d8"><input id="hn_article_url" placeholder="Bronlink, als je die hebt" style="padding:10px;border-radius:9px;border:1px solid #e8e1d8"><textarea id="hn_article_message" rows="3" placeholder="Waarom geef je deze aanvulling door?"></textarea><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px"><input id="hn_article_name" placeholder="Jouw naam (optioneel)" style="padding:10px;border-radius:9px;border:1px solid #e8e1d8"><input id="hn_article_email" type="email" placeholder="E-mail (optioneel)" style="padding:10px;border-radius:9px;border:1px solid #e8e1d8"></div><div><button class="btn btn-primary" type="submit">Bewerkvoorstel insturen</button><span id="hn_article_result" style="margin-left:10px;font-size:12px"></span></div></form></div>';\n    const bottom=view.querySelector('.article-bottom');
